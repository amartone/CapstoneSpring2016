...
#include "ImpedanceMeasurement_2Wire.h"
...
int main(void) {
  ...
  /* set RTC alarm */
  printf("rtcCount: %d\r\n", rtcCount);
  if (ADI_RTC_SUCCESS != adi_RTC_SetAlarm(hRTC, rtcCount + 30))
  ...
  */
  ...
  //DATA_FIFO_EMPTY_STATUS
  
  uint32_t size = 1024;
  fixed32_t magnitudebuffer[1024];
  fixed32_t phasebuffer[1024];
  ...
  if (size) {
          ...
          // Single DMA cycle 
          adi_AFE_ProgramRxDMA(hDevice, &databuffer[0], size);
...
//  unsigned char transmit = (unsigned char) 1;
  while (true/*!done/* nummeasurements < size*/) {
...
//    }
   ...
   // magnitudebuffer[nummeasurements] = magnituderesult;
   // phasebuffer[nummeasurements] = magnituderesult;
...
/*  for(int i; i < size; i ++)
{
  print_MagnitudePhase("", magnitudebuffer[nummeasurements], phasebuffer[nummeasurements]);
}*/
