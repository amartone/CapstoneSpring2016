...
uint32_t seq_afe_acmeas2wire[] = {
    0x00120043,   /* Safety word: bits 31:16 = command count, bits 7:0 = CRC */
...
uint8_t done;
  ...
  int main(void) {
    ...
    while(true/*!done*/){
       // delay(2000000);
       // printf("%8X : %8X\r\n", &(pADI_AFE->AFE_DFT_RESULT_REAL), (pADI_AFE->AFE_DFT_RESULT_REAL));
        //printf("%8X : %d\r\n", &(pADI_AFE->AFE_ADC_RESULT), (pADI_AFE->AFE_ADC_RESULT));
